...
public class SavedAPODDetailsTest extends ApplicationTest {
    ...
    @Test
    public void see_details_of_saved_apod() throws Exception {
        ...
        FxAssert.verifyThat("#grid", NodeMatchers.isNotNull());

        StaggeredGridPane grid = bot.lookup("#grid").query();
        int size = grid.getChildren().size();
        int rndm = ThreadLocalRandom.current().nextInt(1, size+1);
        bot.clickOn("#"+rndm);
        ...
        WaitForAsyncUtils.waitFor(15, TimeUnit.SECONDS, () -> {
            Label titleAPOD = bot.lookup("#titleAPOD").tryQueryAs(Label.class).orElse(null);
            return titleAPOD != null && !titleAPOD.getText().isEmpty();
        });

        APOD fetchedAPOD = repository.findById(rndm);
        Assert.assertEquals(fetchedAPOD.getTitle(), bot.lookup("#titleAPOD").queryAs(Label.class).getText());
    }
